                    ...
                    case '"':
                        {
                            ...
                            for (index = i + 1; index < lispString.Length && !isClosed; index++)
                            {
                                ...
                                switch (cc)
                                {
                                    case '"':
                                        ...
                                        break;
                                    case '\n':
                                        throw new LispParseException("String literal cannot contain a carriage return. Line {0}:{1}", line, position);
                                }
                            }
                            ...
                            if (isClosed)
                            {
                                ...
                                throw new LispParseException("String not closed on line {0}:{1}", line, position);
                        }
                        break;
                    default:
                        {
    ...
    public class LispList : LispItem
    {
        ...
        internal LispList(List<LispToken> tokens, ref int index)
        {
            ...
            if (token.type != TokenType.ParamLeft)
                throw new LispParseException("Lisp List must start with a left parantheses. Line {0}:{1}", token.line, token.position);
