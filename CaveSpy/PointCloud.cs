...
namespace CaveSpy
{
    ...
    public void LoadFromLas(string path)
    {
      ...
      using (var reader = new BinaryReader(fileStream, Encoding.ASCII))
      {
        ...
        switch (_header.PointDataFormat)
        {
          ...
          case 6: _readFunc = PointData.ReadFormat6; break;
          default:
  ...
  class PointCloudHeader
  {
    ...
    void Read(BinaryReader reader)
    {
      ...
      if (this.NumberOfPointRecords == 0)
      {
        this.NumberOfPointRecords = (uint)((reader.BaseStream.Length - reader.BaseStream.Position) / this.PointDataRecordLength);
      }
    }
  }

  /// <summary>
  ///  Specs found at https://www.asprs.org/wp-content/uploads/2010/12/LAS_1_4_r13.pdf
  /// </summary>
  [Serializable]
  class PointData
  {
    ...
    public byte ReturnBits; // 0-2 ReturnNumber, 3-5 NumberOfReturns, 6 Scan Direction Flag, 7 Edge of Flight Line
    private byte Flag;
    ...
    public static void ReadFormat6(PointData p, BinaryReader reader)
    {
      p.X = reader.ReadInt32();
      p.Y = reader.ReadInt32();
      p.Z = reader.ReadInt32();
